...
\end{tikzpicture}
%}
\caption*{\tiny Croisement (crossover)}
\end{figure}
...
\begin{frame}
Soit $p_m=0.1$ la probabilité de mutation et $c_m=0.1$ le coefficient de mutation, pour tout poids $h_i(t)$ du réseau de neurone à l'itération $t$, on tire $U \sim \mathcal{U}(0, 1)$ et $C \sim \mathcal{U}(-1, 1)$. Si $U<p_m$, $h_i(t+1)=h_i(t)+C\times c_m$ sinon $h_i(t+1)=h_i(t)$.
\end{frame}


\begin{frame}
\frametitle{Différentes stratégies de visions et de fitness}
\begin{tikzpicture}
    \begin{axis}[
        width=10cm,
        view={60}{30},
