...
\begin{frame}
  ...
  \vspace{-0.2cm}
  \caption*{\tiny Fonction d'activation sigmoïde}
  \end{figure}
  ...
  %  \readlist\Nnod{4,3,2} % array of number of nodes per layer
    ...
    % LABELS
    \node[above=0.5,align=center,mydarkgreen] at (N1-1.90) {Couche\\[-0.2em]d'entrée};
    \node[above=0.8,align=center,mydarkblue] at (N2-1.90) {Couche\\[-0.2em]cachée};
    \node[above=0.8,align=center,mydarkblue] at (N3-1.90) {Couche\\[-0.2em]cachée};
    \node[above=1.2,align=center,mydarkred] at (N\Nnodlen-1.90) {Couche\\[-0.2em]de sortie};
...
\end{figure}
\footnotesize
\textbf{Remarque:} L'amélioration de la fitness est garantie à chaque génération mais pas la convergence vers un optimum global.
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}
    ...
    \tiny
      ...
      \While{$g < 2000$}
          \For{serpent $s \in \mathcal{P}$}
              \State age $a_s \gets 0$ de $s$, longueur $l_s \gets 3$, point de vie $v_s \gets 50$
              ...
              \While{vivant}
                  ...
                  \If{$\text{position}_s \in \left\{\text{mur}, \text{queue}\right\}$}
                  ...
                  \If{$\text{position}_s \in \left\{\text{pomme}\right\}}$
                      ...
                      \State régénère pomme emplacement aléatoire accessible
                  \Else
                      \State $v_s \gets v_s - 1$
                  \EndIf
                  \If{$v_s<0$}
                      \State $\text{mortVieillesse}_s \gets$ true, $\text{vivant}_s \gets$ false
                  \EndIf
              \EndWhile
              \State $\text{fitness}_s \gets f_s(l_s, a_s, \text{mortVieillesse}_s)$
          \EndFor
          \State $S \gets $les $ 20\%$ meilleurs serpents au sens de $\text{fitness}_s$
          \State Reconstitue $\mathcal{P} \gets S \bigcup \text{mutations}\left[\text{croisements}(\mathcal{P})\right]$
          \State $g \gets g + 1$
      \EndWhile
  \end{algorithmic}
  ...
  \centering
  \vspace{-2.0cm}
  \raisebox{0pt}[\height][\depth]{\hspace{-0.40cm}\includegraphics[width=1.3\textwidth]{snake_population.png}}
  \vspace{-0.7cm}
  ...
  \centering
  \vspace{-1.2cm}
  \raisebox{0pt}[\height][\depth]{\hspace{-0.40cm}
  % NEURAL NETWORK
  ...
  \begin{tikzpicture}[x=5cm,y=1.5cm]
    ...
    \node[above=0.8,align=center,mydarkblue] at (N2-1.90) {Couche\\[-0.2em]cachée};
    \node[above=0.8,align=center,mydarkblue] at (N3-1.90) {Couche\\[-0.2em]cachée};
    \node[above=1.2,align=center,mydarkred] at (N4-1.90) {Couche\\[-0.2em]de sortie};
  \end{tikzpicture}
  }}
  \vspace{-0.7cm}
  \caption*{\tiny Cerveau (stratégie n$\textsuperscript{o}$2)}
  \end{figure}
...
% BEST SNAKES strat 1 22, strat 2 41->47

\section{Annexe I: \textbf{Simulations complémentaires}}

\begin{frame}
  \frametitle{Matrices de poids synaptiques}
  \footnotesize
  \textbf{Cerveau du serpent:} réseau de neurones $\left[n, m, m, k\right]$.

  \textbf{Formulation mathématique:} couche d'entrée vision $n$, deux couches cachées $m$, couche de sortie direction $k$. 
...
\vspace{-0.7cm}
\caption*{\tiny Stratégie vision n$\textsuperscript{o}$1: $\left[24, 18, 18, 4\right]$}
\end{figure}
\end{column}
...
\begin{figure}
  \centering
  \includegraphics[width=1\textwidth]{brain_matrix_13.eps}
\end{figure}
\end{frame}
...
\end{center}
 ...
 \begin{figure}
  \centering
  \includegraphics[width=1\textwidth]{brain_matrix_53.eps}
\end{figure}
\end{frame}
...
\begin{frame}
  ...
  avec 2000 itérations meilleur serpent 51 cases soit plus de la moitier du plateau
  ...
  Éviter les boucle infinie: décrémenter les points de vies, restaurer les points de vie dès qu'on mange une pomme, appliquer une pénalité au score en décrémentant age par 50 si mort de vieillesse
...
\begin{frame}
  \frametitle{Algorithme d'optimisation résultant}
